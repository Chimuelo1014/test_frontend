...
const Motores = () => {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-semibold text-text mb-2">Motores disponibles</h1>
        <p className="text-slate-500">Selecciona un motor de base de datos para crear una instancia</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {motores.map((motor) => (
          <div
            key={motor.id}
            className="bg-white rounded-xl border border-border p-6 hover:shadow-sm transition-all group"
          >
            ...
            <h3 className="text-lg font-semibold text-text mb-1">{motor.name}</h3>
            <p className="text-sm text-slate-500 mb-6">Versión {motor.version}</p>
